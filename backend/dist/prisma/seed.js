...
async function createGeography() {
    ...
    const communes = await Promise.all(communeNames.map((name, index) => prisma.commune.create({
        data: {
            name,
            code: `DK-COM-${index + 1}`,
...
async function createOrganizations(input) {
    const oncav = await prisma.organization.create({
        data: {
            name: 'ONCAV Senegal',
            type: client_1.OrganizationType.ONCAV,
    ...
    const orcav = await prisma.organization.create({
        data: {
            name: 'ORCAV Dakar',
            type: client_1.OrganizationType.ORCAV,
            parentId: oncav.id,
    ...
    const odcav = await prisma.organization.create({
        data: {
            name: 'ODCAV Dakar',
            type: client_1.OrganizationType.ODCAV,
            parentId: orcav.id,
            regionId: input.regionId,
    ...
    const zones = await Promise.all(zoneNames.map((name, index) => prisma.organization.create({
        data: {
            name,
...
async function createUsers(input) {
    ...
    const users = [
        ...
        },
        {
            ...
            role: client_1.Role.ONCAV_ADMIN,
        ...
        },
        {
            ...
            role: client_1.Role.ORCAV_ADMIN,
        ...
        },
        {
            ...
            role: client_1.Role.ODCAV_ADMIN,
        ...
        },
        {
            ...
            role: client_1.Role.GUICHET_AGENT,
            organizationIds: [],
        },
        {
    ...
    for (const assignment of agentAssignments) {
        ...
        await prisma.zoneAgentAssignment.create({
            data: {
                ...
                assignmentType: client_1.AssignmentType.BOTH,
            },
        });
    }
    return {
...
async function createVenuesAndTeams(zoneIds) {
    ...
    for (const [index, zoneId] of zoneIds.entries()) {
        ...
        const homeTeam = await prisma.team.create({
            data: {
                organizationId: zoneId,
                name: `ASC ${index + 1} A`,
                category: client_1.AgeCategory.SENIOR,
            },
        });
...
async function createMatches(input) {
    ...
    for (let index = 0; index < 20; index += 1) {
        ...
        const createdById = input.zoneAdminIds[index % input.zoneAdminIds.length] ?? input.zoneAdminIds[0];
        if (!venue || !teamPair) {
            continue;
        }
        ...
        const categoryBlueprints = [
            {
                name: 'Populaire',
                price: new client_1.Prisma.Decimal(basePrice),
                quota: 320 + index * 6,
                badgeColor: '#0F766E',
            },
            {
                name: 'Tribune',
                price: new client_1.Prisma.Decimal(basePrice + 500),
                quota: 140 + index * 3,
                badgeColor: '#D97706',
            },
            {
                name: 'VIP',
                price: new client_1.Prisma.Decimal(basePrice + 1500),
                quota: 40 + (index % 5) * 5,
                badgeColor: '#7C3AED',
            },
        ];
...
async function createOrdersTicketsAndScans(input) {
    ...
    for (const [index, match] of input.matches.entries()) {
        ...
        for (let orderIndex = 0; orderIndex < paidOrdersForMatch; orderIndex += 1) {
            const category = match.ticketCategories[orderIndex % match.ticketCategories.length] ??
                match.ticketCategories[0];
            const quantity = orderIndex === 0 ? 3 : 2;
            const totalAmount = new client_1.Prisma.Decimal(category.price).mul(quantity);
            ...
            await prisma.payment.create({
                data: {
                    orderId: order.id,
                    provider: orderIndex % 2 === 0
                        ? client_1.PaymentProvider.WAVE_MOCK
                        : client_1.PaymentProvider.ORANGE_MONEY_MOCK,
            ...
            for (let ticketIndex = 0; ticketIndex < quantity; ticketIndex += 1) {
                ...
                const ticket = await prisma.ticket.create({
                    data: {
                        ...
                        status: isUsed ? client_1.TicketStatus.USED : client_1.TicketStatus.GENERATED,
                ...
                if (isUsed) {
                    ...
                    await prisma.ticketScan.create({
                        data: {
                            ...
                            matchId: match.id,
                            scannedById,
                            scanResult: client_1.ScanResult.ALREADY_USED,
