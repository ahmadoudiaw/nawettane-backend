...
let ReportingService = ReportingService_1 = class ReportingService {
    ...
    async getDashboard(user, query) {
        ...
        try {
            ...
            return {
                ...
                revenue: revenue._sum.totalAmount ?? new client_1.Prisma.Decimal(0),
                ticketsScanned: scans.length,
                ticketsUnused: tickets.filter((ticket) => ticket.status !== client_1.TicketStatus.USED).length,
                rows,
            };
        }
        catch (error) {
            this.logger.error(`Grouped reporting failed for reportType=${reportType}; falling back to legacy KPI dashboard.`, error instanceof Error ? error.stack : String(error));
            return this.getLegacyDashboard(matchWhere, query, reportType);
        }
    }
    async exportMatchesWorkbook(user, query) {
    ...
    async exportSalesByMatchWorkbook(user, query) {
    ...
    async exportTicketsWorkbook(user, query) {
    ...
    async exportPaymentsWorkbook(user, query) {
    ...
    async exportRevenueWorkbook(user, query) {
    ...
    async exportDashboardWorkbook(user, query) {
    ...
    async getAnalytics(user, query) {
        const scope = user.scope ?? (0, scope_util_1.buildScopeContext)(user);
        const matchWhere = this.buildMatchWhere(scope, query);
        let ordersWithCategory = [];
        try {
            const rawOrders = await this.prisma.order.findMany({
                where: { match: matchWhere, status: client_1.OrderStatus.PAID },
                select: {
                    matchId: true,
                    quantity: true,
                    totalAmount: true,
        ...
        for (const order of ordersWithCategory) {
            ...
            if (!catMap.has(name))
                catMap.set(name, { name, ticketsSold: 0, revenue: 0 });
    ...
    async getLegacyDashboard(matchWhere, query, reportType) {
        const orderWhere = { match: matchWhere };
        const ticketWhere = { match: matchWhere };
        const scanWhere = { match: matchWhere };
    ...
    buildMatchWhere(scope, query) {
        ...
        const filterWhere = {
            ...
            organization: query.regionId || query.departmentId || query.communeId
                ...
                : undefined,
            matchDate: query.fromDate || query.toDate || query.week
                ? {
    ...
    buildRows(reportType, matches, orders, tickets, scans) {
        const rows = new Map();
    ...
    async getScopedMatches(user, query) {
        const scope = user.scope ?? (0, scope_util_1.buildScopeContext)(user);
        const matchWhere = this.buildMatchWhere(scope, query);
    ...
    async getScopedPaidOrders(user, query) {
        const scope = user.scope ?? (0, scope_util_1.buildScopeContext)(user);
        const matchWhere = this.buildMatchWhere(scope, query);
    ...
    async getScopedTickets(user, query) {
        const scope = user.scope ?? (0, scope_util_1.buildScopeContext)(user);
        const matchWhere = this.buildMatchWhere(scope, query);
        try {
            ...
            this.logger.warn('Ticket category table is unavailable for reporting exports; falling back to legacy ticket category values.');
    ...
    async getScopedPayments(user, query) {
        const scope = user.scope ?? (0, scope_util_1.buildScopeContext)(user);
        const matchWhere = this.buildMatchWhere(scope, query);
